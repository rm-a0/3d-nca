"""
ObjectConverter - OBJ file I/O and voxelization utilities.

Converts OBJ mesh files to voxelized tensor representations suitable for NCA training.
Handles automatic mesh centering, scaling, color extraction, and format conversion
between external (D,H,W,C) and internal (B,C,D,H,W) tensor layouts.
"""
...
def obj_to_tensor(
    ...
    if voxel_matrix.shape != grid_size:
        ...
        target_grid[slices_tgt[0], slices_tgt[1], slices_tgt[2]] = voxel_matrix[
            slices_src[0], slices_src[1], slices_src[2]
        ]
    ...
    if mode == "rgba":
        ...
        tensor = np.stack(
            [r_channel, g_channel, b_channel, a_channel], axis=0
        )  # (C=4, D, H, W)
    elif mode == "alpha":
